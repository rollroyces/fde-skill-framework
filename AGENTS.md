...
This file is auto-discovered by Codex CLI and other agents that
follow the AGENTS.md convention. It mirrors the FDE workflow
skill so every agent in this repo behaves the same way.

> Source of truth: `.hermes/skills/fde-workflow.md`. Re-generate
> via `python scripts/build_adapters.py` after editing the source.
...
## Pointer to scoring harness

Any agent in this repo can score an engagement log by running:

```bash
python -m fde score <engagement>.log.json
```

The harness is a black-box evaluator (`tests/test_fde_eval.py`)
that scores on four weighted axes (Business 35% / SLA 30% /
Latency trend 15% / Error rate 20%) and returns a decision:
`scale | iterate | cut`.
